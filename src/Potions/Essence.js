class Essence {
  ...
  static calculateMod(effectArray) {
    ...
    if (
      effectArray.every(
        element =>
          element.potency === effectArray[0].potency && effectArray.length === 2
      )
    ) {
      ...
      effectArray.every(
        element =>
          element.potency === effectArray[0].potency && effectArray.length === 3
      )
    ) {
      ...
      effectArray.every(
        element =>
          element.potency === effectArray[0].potency && effectArray.length === 4
      )
    ) {
